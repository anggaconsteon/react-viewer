...
| **draft-carry (customer+vehicle+items)** | P1→P4 | **HIGH** | ⚠ live-test 2026-06-30: **cuma `it[]` yg nyampe P4** (manifest jalan; customer/vehicle BLANK, `{kl}` gak ke-carry). Bawa customer `kl/kn/al/pic` + vehicle `vv` **JUGA** (bukan items). **DECIDED: carry via DRAFT** (P1 list-tap set `draft.customer`), bukan routeParams. | create-task §0.3/§5 | — |
| `CONTEXT_CARD` | P2·P3·P4 (+reuse) | MED | **NEW** generic — baca `draft` entity → kartu (icon/title/meta/sub) + banner count/seed (query badgeTable). Nampilin customer di tiap step (mockup). Header jadi title-only. | create-task §0.3 | Widget @250 |
