...
5. **Seberapa sering kamar Lodge berganti penghuni?** Ini pemicu untuk §8.1 (serah-terima). Sewa bulanan tapi rata-rata tinggal setahun = jarang; kalau tiap 2–3 bulan, gapnya menganga lebih cepat.
...
8. **Nomor seri meter direkam sekali saat pendataan, lalu cuma DITAMPILKAN untuk dicocokkan mata**
   **8b. NOMOR SERI DIBACA OCR DARI FOTO — tidak diketik, tidak ditranskripsi manual (revisi 19 Agu).** Aturan payungnya: **"jangan minta orang mengetik di lapangan sesuatu yang bisa dibaca dari fotonya nanti."** Nomor seri tercetak di muka/badan meter, jadi ikut terekam di foto yang sama. Di registrasi, OCR yang merekamnya; di pembacaan bulanan, OCR mencocokkannya dengan yang terdaftar dan menampilkan salah satu dari empat keadaan: **cocok · beda · tidak ada · tidak terbaca**. Yang "beda" otomatis menandai bacaan untuk ditinjau, tanpa petugas menekan apa pun. Strip "nomor meter terdaftar" di layar kamera dan tombol "nomornya beda" **dibuang** — tidak ada lagi yang perlu dicocokkan mata.
   **8b-i. BATAS OCR YANG DIPERTAHANKAN: OCR boleh mengisi IDENTITAS, tidak boleh mengisi ANGKA YANG DITAGIHKAN.** Bedanya ada di cara rusaknya — OCR salah baca nomor seri cuma memicu satu tinjauan yang tidak perlu; OCR salah baca angka meter membuat tagihan orang salah. Jadi nomor seri diisi mesin, angka meter tetap disalin petugas dengan OCR sebagai pemeriksa.
...
15b. **Hasil pemeriksaan muncul sebagai BOTTOM SHEET, dan hanya kalau bermasalah** (revisi 19 Agu). Kalau angkanya cocok, tidak ada apa pun yang muncul — jalur normal nol gangguan, cukup baris ringkas. Sheet-nya bisa ditutup tanpa kehilangan status: yang tersisa baris ringkas yang bisa ditap untuk membukanya lagi. Ini menjawab kekhawatiran awal (orangnya mungkin harus jalan ke meter lain sambil layarnya kehalang) tanpa mengorbankan pola bottom sheet.
...
22. **~~Tiga tingkat keyakinan pemetaan~~ — DICABUT (19 Agu).** Dirancang waktu masih diduga meternya berderet di satu panel; ternyata tiap ruko punya carport sendiri, jadi pertanyaannya punya satu jawaban yang sama untuk 300 unit. **Pertanyaan yang jawabannya selalu sama melatih orang menekan tanpa membaca** — persis alasan yang dipakai untuk menolak OCR-mengisi-kolom, jadi mempertahankannya berarti tidak konsisten. Keraguan pemetaan sekarang ditangkap **hanya saat penjaga seed bersuara**, yaitu satu-satunya momen keraguan itu benar-benar ada.
    **22b. Jalan keluar jujur dari blokir: "meter ini baru diganti".** Meter pengganti mulai dari nol, jadi angkanya wajar lebih kecil dari seed. Tanpa jalur ini, blokirnya memaksa orang mengarang angka supaya bisa lanjut — dan **blokir yang memaksa orang berbohong lebih buruk daripada tidak ada blokir**. Unit yang ditandai begini kehilangan seed sebagai pembanding, dan itu dicatat.
...
## 5. Dua layar petugas yang berbeda — dan usulan penamaan

Ada **dua** layar yang sama-sama dipakai petugas di lapangan, dan namanya sekarang terlalu mirip (`PendataanMeter` vs `PencatatanMeter` — beda dua huruf, beda pekerjaan). **Usulan ganti nama: `RegistrasiMeter.jsx` dan `PembacaanMeter.jsx`.**

| | **Registrasi** (putaran nol) | **Pembacaan** (bulanan) |
| --- | --- | --- |
| Kapan | **Sekali** seumur hidup meter | **Tiap bulan**, selamanya |
| Berapa kali | ~400× total | ~400× per bulan |
| Tujuannya | Menetapkan **identitas** meter & titik awal | Menangkap **angka bulan ini** |
| Pembandingnya | Seed spreadsheet Paskal | Bacaan bulan lalu di sistem |
| Nomor seri | **Direkam** pertama kali (OCR) | **Dicocokkan** dengan yang terdaftar |
| Jumlah digit | Bisa dikoreksi per meter ("beda?") | Terkunci dari config site |
| Urutan keliling | **Dihasilkan** dari urutan kerja | **Dipakai** sebagai urutan |
| Kalau angka mundur | **MEMBLOKIR** + jalan keluar "meter diganti" | Tidak memblokir — petugas menang, cuma ditandai |
| Foto | Angka **+ nomor seri** diusahakan satu frame | Angka saja |
| "Tidak bisa dibaca" | **Belum ada** — lihat celah di bawah | Ada, alasan wajib |

**Kenapa blokirnya cuma di registrasi:** di situ yang ditetapkan adalah *identitas*, dan salah identitas berulang tiap bulan tanpa bisa ditangkap pemeriksaan mana pun. Di pembacaan bulanan yang dipertaruhkan cuma *satu angka* — bisa dikoreksi bulan depan, dan koreksinya berjejak.

**Celah yang belum ditutup:** registrasi belum punya jalur "meter tidak bisa dibaca / tidak ditemukan". Di lapangan itu pasti terjadi (box terkunci, meter hilang), dan sekarang satu-satunya jalan keluarnya adalah tidak menyimpan sama sekali — yang berarti unit itu diam-diam terlewat dari daftar. Perlu ditambahkan sebelum putaran nol beneran dijalankan.

---

## 6. Materi yang sudah jadi
...
| `PendataanMeter.jsx` + `autsorz_pendataan_meter.html` | **Registrasi meter** (putaran nol): penjaga pemetaan lewat seed, OCR nomor seri, jumlah digit inline, jalan keluar "meter diganti" | Diuji semua cabang · usul ganti nama jadi `RegistrasiMeter.jsx` |
...
### 8.1 Serah-terima kamar Lodge — DITUNDA (keputusan owner, 18 Agu)
...
## 10. Cara kerja owner (untuk sesi baru)
