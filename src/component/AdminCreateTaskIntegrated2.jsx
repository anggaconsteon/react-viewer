...
export const PRODUCT_CATALOG = [
    { id: "gas_12", name: "Gas 12kg", type: "returnable", category: "Gas", icon: "🔥", harga: 215000 },
    { id: "gas_3", name: "Gas 3kg", type: "returnable", category: "Gas", icon: "🔥", harga: 22000 },
    { id: "aqua_galon", name: "Aqua Galon", type: "returnable", category: "Water", icon: "💧", harga: 20000 },
    { id: "pristine_galon", name: "Pristine Galon", type: "returnable", category: "Water", icon: "💧", harga: 22000 },
    { id: "aqua_600", name: "Aqua 600ml (Dus)", type: "consumable", category: "Water", icon: "💧", harga: 48000 },
    { id: "aqua_1500", name: "Aqua 1500ml (Dus)", type: "consumable", category: "Water", icon: "💧", harga: 60000 },
    { id: "leminerale_600", name: "Le Minerale 600ml (Dus)", type: "consumable", category: "Water", icon: "💧", harga: 52000 },
];

// harga = atribut product (B2). Sumber tunggal harga; di-snapshot (beku) ke nota saat terbit.
// Editor harga = settings produk GLOBAL (di luar file ini). Di sini hanya BACA.
// Refill: harga isi galon dihitung TERPISAH (parkir) — bukan PRODUCT_CATALOG.harga.
                ...
                <div style={{ flex: 1, overflowY: "auto", padding: "8px 14px 14px" }} className="scroll-thin">
                    {available.length === 0 ? (
                        ...
                        <button key={p.id} onClick={() => onSelect(p)} className="tap-feedback" style={{
                            ...
                            <span style={{ fontSize: 20 }}>{p.icon}</span>
                            <span style={{ flex: 1, fontSize: 14, fontWeight: 600, color: C.text }}>{p.name}</span>
                            <span className="mono" style={{ fontSize: 12.5, fontWeight: 700, color: C.sale700 }}>{rp(p.harga)}</span>
                            <span style={{ color: C.textDim, fontSize: 16 }}>›</span>
                        </button>
                    ))}
                </div>
...
export const WalkInIntakeScreen = ({ items, onUpdate, fulfillment, onFulfillment, buyer, onBuyer, onBack, onProceed }) => {
    ...
    const addConsumable = (p) => {
        onUpdate([...items, { itemId: p.id, name: p.name, icon: p.icon, type: "consumable", transactionType: "SALE", drop: 1, harga: p.harga }]);
    ...
    const setHarga = (id, v) => onUpdate(items.map(i => i.itemId === id ? { ...i, harga: Math.max(0, v) } : i)); // override per-baris; default = katalog
    ...
    const totalRp = items.reduce((s, i) => i.transactionType === "SALE" ? s + (i.harga || 0) * (i.drop || 0) : s, 0);
    ...
    return (
        <div style={{ height: "100%", display: "flex", flexDirection: "column", background: C.bg }}>
                ...
                {items.map(item => {
                    ...
                    return (
                        <div key={item.itemId} style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: "12px", marginBottom: 10 }}>
                            ...
                            {item.harga != null && (() => {
                                const catalogHarga = (PRODUCT_CATALOG.find(p => p.id === item.itemId) || {}).harga;
                                const edited = catalogHarga != null && item.harga !== catalogHarga;
                                return (
                                    <div style={{ marginTop: 10 }}>
                                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                            <span style={{ fontSize: 12, color: C.textMid }}>Harga</span>
                                            <div style={{ display: "flex", alignItems: "center", gap: 4, background: C.surfaceAlt, border: `1px solid ${edited ? C.sale400 : C.border}`, borderRadius: 8, padding: "6px 10px" }}>
                                                <span style={{ fontSize: 12, color: C.textDim }}>Rp</span>
                                                <input
                                                    type="text" inputMode="numeric"
                                                    value={(item.harga || 0).toLocaleString("id-ID")}
                                                    onChange={(e) => setHarga(item.itemId, Number(e.target.value.replace(/[^\d]/g, "")))}
                                                    className="mono"
                                                    style={{ width: 74, border: "none", background: "transparent", textAlign: "right", fontSize: 13, fontWeight: 700, color: C.text, outline: "none" }}
                                                />
                                            </div>
                                            <span style={{ flex: 1 }} />
                                            <span className="mono" style={{ fontSize: 13, fontWeight: 700, color: C.text }}>{rp((item.harga || 0) * (item.drop || 0))}</span>
                                        </div>
                                        {edited && (
                                            <div style={{ marginTop: 5, display: "flex", alignItems: "center", gap: 8, fontSize: 10.5, color: C.sale700 }}>
                                                <span>Harga diubah · katalog {rp(catalogHarga)}</span>
                                                <button onClick={() => setHarga(item.itemId, catalogHarga)} className="tap-feedback" style={{ background: "transparent", border: "none", color: C.adminAccent, fontSize: 10.5, fontWeight: 700, cursor: "pointer", padding: 0 }}>reset</button>
                                            </div>
                                        )}
                                    </div>
                                );
            ...
            <div style={{ background: C.surface, borderTop: `1px solid ${C.border}`, padding: "10px 14px 12px", flexShrink: 0, boxShadow: "0 -4px 12px rgba(0,0,0,0.04)" }}>
                {items.length > 0 && (
                    <div style={{ display: "flex", alignItems: "center", marginBottom: 10, padding: "9px 12px", background: C.surfaceAlt, borderRadius: 8, fontSize: 12 }}>
                        <span style={{ color: C.textMid }}>{totalQty} item</span>
                        {hasRefill && <span style={{ color: C.textDim, fontSize: 11, marginLeft: 6 }}>· refill harga terpisah</span>}
                        <span style={{ flex: 1 }} />
                        <span className="mono" style={{ color: C.sale700, fontWeight: 700, fontSize: 14 }}>{rp(totalRp)}</span>
                    </div>
                )}
...
export const WalkInCounterSuccess = ({ items, buyer, taskId, onDone }) => {
    // Counter = bayar di tempat → SELALU LUNAS. Metode hanya tunai/transfer.
    const [metode, setMetode] = useState("tunai");
    const [printed, setPrinted] = useState(false);
    const saleItems = items.filter(i => i.transactionType === "SALE");
    const refillItems = items.filter(i => i.transactionType === "REFILL");
    // Total = SALE saja. Harga refill (isi galon milik customer) = perhitungan terpisah (parkir).
    const totalRp = saleItems.reduce((s, i) => s + (i.harga || 0) * (i.drop || 0), 0);

    return (
        <div style={{ height: "100%", display: "flex", flexDirection: "column", background: C.bg }}>
            <div style={{ background: C.surface, borderBottom: `1px solid ${C.border}`, padding: "12px 14px", flexShrink: 0 }}>
                <div style={{ fontSize: 11, color: C.sale700, textTransform: "uppercase", letterSpacing: "0.04em", fontWeight: 700 }}>🚶 Walk-in · Counter</div>
                <div style={{ fontSize: 16, fontWeight: 700, color: C.text }}>Nota Penjualan</div>
            </div>

            <div style={{ flex: 1, overflowY: "auto", padding: "14px 14px 16px" }} className="scroll-thin">
                {/* Pembayaran — counter = selalu LUNAS, pilih metode */}
                <div style={{ fontSize: 11, color: C.textDim, textTransform: "uppercase", letterSpacing: "0.06em", fontWeight: 700, marginBottom: 8 }}>Pembayaran</div>
                <div style={{ display: "flex", gap: 0, marginBottom: 16, background: C.surfaceAlt, border: `1px solid ${C.border}`, borderRadius: 8, padding: 3 }}>
                    {[{ id: "tunai", label: "Tunai" }, { id: "transfer", label: "Transfer" }].map(opt => {
                        const active = metode === opt.id;
                        return (
                            <button key={opt.id} onClick={() => setMetode(opt.id)} className="tap-feedback" style={{
                                flex: 1, padding: "9px 8px", borderRadius: 6, border: "none", cursor: "pointer",
                                background: active ? C.surface : "transparent", boxShadow: active ? "0 1px 2px rgba(0,0,0,0.06)" : "none",
                                fontSize: 13, fontWeight: 700, color: active ? C.text : C.textMid,
                            }}>{opt.label}</button>
                        );
                    })}
                </div>

                {/* Struk nota — render 80mm thermal (konsisten dgn Document Engine) */}
                <div style={{ display: "flex", justifyContent: "center" }}>
                    <div style={{ width: 292, filter: "drop-shadow(0 8px 20px rgba(0,0,0,0.10))" }}>
                        <div style={{ height: 9, background: `linear-gradient(135deg, transparent 50%, #FCFCFA 50%) 0 0 / 9px 9px, linear-gradient(-135deg, transparent 50%, #FCFCFA 50%) 0 0 / 9px 9px` }} />
                        <div className="mono" style={{ background: "#FCFCFA", padding: "15px 16px 16px", color: C.text, position: "relative" }}>
                            <div style={{ textAlign: "center" }}>
                                <div style={{ fontSize: 13.5, fontWeight: 700, letterSpacing: 0.5 }}>DEPO TIRTA JAYA</div>
                                <div style={{ fontSize: 10, color: C.textMid, marginTop: 2 }}>Jl. Merdeka No. 12, Ciputat</div>
                                <div style={{ fontSize: 10, color: C.textMid }}>Telp 0821-1100-4477</div>
                            </div>
                            <div style={{ borderTop: `1px dashed ${C.textDim}`, margin: "9px 0" }} />
                            <div style={{ textAlign: "center", fontSize: 12, fontWeight: 700, letterSpacing: 2 }}>NOTA PENJUALAN</div>
                            <div style={{ marginTop: 7, fontSize: 11 }}>
                                <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ color: C.textMid }}>No.</span><span>{taskId}</span></div>
                                <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ color: C.textMid }}>Pembeli</span><span>{buyer.name || "Umum"}</span></div>
                            </div>
                            <div style={{ borderTop: `1px dashed ${C.textDim}`, margin: "9px 0" }} />
                            {saleItems.map(i => (
                                <div key={i.itemId} style={{ marginBottom: 5 }}>
                                    <div style={{ fontSize: 11.5, fontWeight: 600 }}>{i.name}</div>
                                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: C.textMid }}>
                                        <span>{i.drop} × {rp(i.harga)}</span>
                                        <span style={{ color: C.text, fontWeight: 600 }}>{rp((i.harga || 0) * (i.drop || 0))}</span>
                                    </div>
                                </div>
                            ))}
                            {refillItems.map(i => (
                                <div key={i.itemId} style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: C.textMid, marginBottom: 4 }}>
                                    <span>{i.name} (refill ⇄{i.refillQty})</span><span>harga terpisah</span>
                                </div>
                            ))}
                            <div style={{ borderTop: `1px dashed ${C.textDim}`, margin: "9px 0" }} />
                            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, fontWeight: 700 }}><span>TOTAL</span><span>{rp(totalRp)}</span></div>
                            <div style={{ borderTop: `1px dashed ${C.textDim}`, margin: "9px 0" }} />
                            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}><span style={{ color: C.textMid }}>{metode === "tunai" ? "Tunai" : "Transfer"}</span><span style={{ fontWeight: 700 }}>{rp(totalRp)}</span></div>
                            <div style={{ textAlign: "center", fontSize: 10, color: C.textMid, marginTop: 10 }}>Terima kasih 🙏</div>
                            <div style={{ textAlign: "center", fontSize: 9, color: C.textDim, letterSpacing: 1, marginTop: 8 }}>consteon</div>
                            <div style={{ position: "absolute", top: 152, right: 12, border: `2.5px solid ${C.emerald700}`, color: C.emerald700, borderRadius: 7, padding: "2px 10px", fontSize: 16, fontWeight: 800, letterSpacing: 1.5, transform: "rotate(-12deg)", opacity: 0.9, background: "rgba(16,185,129,0.07)" }}>LUNAS</div>
                        </div>
                        <div style={{ height: 9, background: `linear-gradient(135deg, transparent 50%, #FCFCFA 50%) 0 0 / 9px 9px, linear-gradient(-135deg, transparent 50%, #FCFCFA 50%) 0 0 / 9px 9px`, transform: "rotate(180deg)" }} />
                    </div>
                </div>
                {refillItems.length > 0 && (
                    <div style={{ marginTop: 10, fontSize: 10.5, color: C.textDim, lineHeight: 1.4, textAlign: "center" }}>
                        Harga refill (isi galon milik customer) dihitung terpisah — belum masuk total nota.
                    </div>
                )}

                <div style={{ marginTop: 10, fontSize: 10.5, color: C.textDim, lineHeight: 1.5 }}>
                    Harga & status bayar = fakta di nota ini (beku). Stok gudang berkurang · tanpa custody. Tanpa piutang/saldo.
                </div>
            </div>

            <div style={{ background: C.surface, borderTop: `1px solid ${C.border}`, padding: "10px 14px 12px", flexShrink: 0, display: "flex", gap: 8 }}>
                <button onClick={() => setPrinted(true)} className="tap-feedback" style={{
                    flex: 1, background: printed ? C.emerald50 : C.sale600, color: printed ? C.emerald700 : "#fff",
                    border: printed ? `1px solid ${C.emerald400}` : "none", padding: "14px", borderRadius: 10, fontSize: 13.5, fontWeight: 700, cursor: "pointer",
                }}>{printed ? "✓ Nota Tercetak" : "Cetak Nota"}</button>
                <button onClick={onDone} className="tap-feedback" style={{
                    flex: 1, background: "transparent", color: C.adminAccent, border: `1px solid ${C.adminAccent}`, padding: "14px", borderRadius: 10, fontSize: 13.5, fontWeight: 700, cursor: "pointer",
