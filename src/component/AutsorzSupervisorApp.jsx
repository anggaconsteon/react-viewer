...
import {
    ...
    Crosshair, Plus, Printer, Share2, Minus, Navigation, Building2,
} from "lucide-react";
...
// =================== TITIK ABSENSI (fitur baru) ===================
// Supervisor menentukan titik absensi per site: peta (deteksi GPS device
// ATAU ketik alamat), radius, lalu QR digenerate utk ditempel di lokasi.
// PENTING (dari diskusi): ini momen kelahiran koordinat site di AUTSORZ.
// Sebelum ini site cuma nama; dari sini site punya koordinat yg nanti bisa
// diwarisi titik patroli/cleaning.
//
// Integritas (bagian 2): radius = KONTEKS TAMBAHAN, bukan gerbang tunggal.
// Scan QR + foto tetap bukti utama. Radius cuma menjawab "di lokasi benar?".
//
// Radius default & batas — config, bukan hardcode makna.
const RADIUS_MIN = 30;
const RADIUS_MAX = 300;
const RADIUS_DEFAULT = 100;
const RADIUS_STEP = 10;

// Koordinat dummy per site (di produk: ditangkap dari GPS/geocoding).
// Beberapa site sudah punya titik, sebagian belum (demo dua kondisi).
const INITIAL_ATTENDANCE_POINTS = {
    "bp-legok": [
        {
            id: "ap-1", label: "Pos Utama Lobby", lat: -6.2712, lng: 106.6295,
            radius: 80, address: "Gedung BP Legok, Lobby Utama", qrReady: true
        },
        {
            id: "ap-2", label: "Pos Belakang", lat: -6.2718, lng: 106.6301,
            radius: 60, address: "Gedung BP Legok, Area Loading Dock", qrReady: true
        },
    ],
    "goto-tangerang": [
        {
            id: "ap-3", label: "Lobby Tower", lat: -6.2301, lng: 106.6512,
            radius: 100, address: "GoTo Tower, Lobby Lt 1", qrReady: true
        },
    ],
    // Site lain belum punya titik (demo kondisi kosong)
};
...
function ScreenHome({ approvals, incidents, complaints, onGo }) {
    ...
    return (
        <div>
            ...
            {/* Kartu Titik Absensi — menu terpisah (fitur baru) */}
            <HomeCard
                icon={MapPin}
                tone="neutral"
                label="Titik Absensi"
                headline="Atur titik & QR absensi"
                details="Tentukan lokasi + radius, generate QR untuk ditempel di lokasi"
                onClick={() => onGo("attendance_points")}
            />
...
function HomeCard({ icon: Icon, tone, label, headline, details, onClick }) {
    const toneColor = { danger: C.danger, warn: C.warn, ok: C.ok, neutral: C.blue }[tone];
    const toneBg = { danger: C.dangerBg, warn: C.warnBg, ok: C.okBg, neutral: C.blueBg }[tone];
    return (
        <button onClick={onClick}
            style={{
...
// ============================================================
//  TITIK ABSENSI — LAYAR SUPERVISOR (fitur baru)
//  Alur: home → daftar site (jumlah titik) → titik per site →
//  form (peta + radius) → QR preview.
// ============================================================

// ----- Representasi peta ringan (bukan peta asli) -----
function MapRepresentation({ lat, lng, radius, locating }) {
    const px = Math.max(40, Math.min(180, radius * 0.9));
    return (
        <div style={{
            position: "relative", width: "100%", height: 260,
            background: "#EAF0F4", borderRadius: 14, overflow: "hidden",
            border: `1.5px solid ${C.cardBd}`
        }}>
            <div style={{
                position: "absolute", inset: 0,
                backgroundImage: `linear-gradient(${C.cardBd} 1px, transparent 1px),
          linear-gradient(90deg, ${C.cardBd} 1px, transparent 1px)`,
                backgroundSize: "28px 28px", opacity: 0.5
            }} />
            <div style={{
                position: "absolute", top: "45%", left: 0, right: 0, height: 12,
                background: "#DDE4EA"
            }} />
            <div style={{
                position: "absolute", top: 0, bottom: 0, left: "58%", width: 12,
                background: "#DDE4EA"
            }} />
            <div style={{
                position: "absolute", top: "50%", left: "50%",
                width: px, height: px, transform: "translate(-50%, -50%)",
                borderRadius: "50%", background: `${C.blue}22`,
                border: `2px solid ${C.blue}`, transition: "width 0.15s, height 0.15s"
            }} />
            <div style={{
                position: "absolute", top: "50%", left: "50%",
                ...
                <MapPin size={30} color={C.danger} fill={C.danger} strokeWidth={1.5} />
            </div>
            <div style={{
                ...
                {locating ? "Mendeteksi lokasi…" : `${lat.toFixed(4)}, ${lng.toFixed(4)}`}
            </div>
            <div style={{
                position: "absolute", right: 10, top: 10,
                background: C.blue, color: "#fff", borderRadius: 8, padding: "5px 10px",
...
function ScreenAttendancePoints({ points, onOpenSite }) {
    return (
        <div>
            <p style={{ fontSize: 13.5, color: C.mute, margin: "0 0 14px", lineHeight: 1.5 }}>
                Tentukan titik absensi tiap site. Titik jadi dasar scan QR absen —
                worker scan QR yang Anda tempel di lokasi.
            </p>
            {SITES.map((site) => {
                const sitePoints = points[site.id] || [];
                const count = sitePoints.length;
                return (
                    <button key={site.id} onClick={() => onOpenSite(site.id)}
                        style={{
                            width: "100%", display: "flex", alignItems: "center", gap: 12,
                            padding: "13px 14px", background: "#fff",
                            border: `1.5px solid ${C.cardBd}`, borderRadius: 13,
                            marginBottom: 9, cursor: "pointer", fontFamily: "inherit", textAlign: "left"
                        }}>
                        <span style={{
                            width: 38, height: 38, borderRadius: 10, background: C.chip,
                            display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0
                        }}>
                            <Building2 size={18} color={C.mute} />
                        </span>
                        <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontSize: 14.5, fontWeight: 700, color: C.ink }}>{site.name}</div>
                            <div style={{
                                fontSize: 12, marginTop: 2,
                                color: count > 0 ? C.ok : C.soft
                            }}>
                                {count > 0 ? `${count} titik absensi` : "Belum ada titik"}
                            </div>
                        </div>
                        <ChevronRight size={18} color="#C4C8D0" />
                    </button>
...
function ScreenSitePoints({ site, points, onAdd, onEdit, onOpenQR }) {
    const sitePoints = points[site.id] || [];
    return (
        <div>
            <div style={{
                ...
                <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 15, fontWeight: 800, color: C.ink }}>{site.name}</div>
                    <div style={{ fontSize: 11.5, color: C.soft, marginTop: 2 }}>
                        {sitePoints.length} titik absensi
                    </div>
                </div>
            </div>
            ...
            {sitePoints.length === 0 && (
                <div style={{
                    textAlign: "center", padding: "26px 20px", color: C.soft,
                    background: "#fff", border: `1.5px dashed ${C.cardBd}`, borderRadius: 13,
                    marginBottom: 14
                }}>
                    <MapPin size={26} style={{ opacity: 0.5 }} />
                    <div style={{ fontSize: 13, marginTop: 8, lineHeight: 1.5 }}>
                        Belum ada titik absensi.<br />Tambah titik pertama untuk site ini.
                    </div>
                </div>
            )}
            ...
            {sitePoints.map((pt) => (
                <div key={pt.id}
                    style={{
                        background: "#fff", border: `1.5px solid ${C.cardBd}`,
                        borderRadius: 13, padding: "13px 14px", marginBottom: 10
                    }}>
                    <div style={{ display: "flex", alignItems: "flex-start", gap: 11 }}>
                        <span style={{
                            width: 34, height: 34, borderRadius: 9, background: C.blueBg,
                            display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0
                        }}>
                            <MapPin size={16} color={C.blue} />
                        </span>
                        <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontSize: 14, fontWeight: 700, color: C.ink }}>{pt.label}</div>
                            <div style={{ fontSize: 11.5, color: C.mute, marginTop: 2, lineHeight: 1.4 }}>
                                {pt.address}
                            </div>
                            <div style={{
                                fontSize: 11, color: C.soft, marginTop: 4,
                                display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap"
                            }}>
                                <span style={{ display: "inline-flex", alignItems: "center", gap: 3 }}>
                                    <Navigation size={10} /> {pt.lat.toFixed(4)}, {pt.lng.toFixed(4)}
                                </span>
                                <span style={{ display: "inline-flex", alignItems: "center", gap: 3 }}>
                                    <Crosshair size={10} /> radius {pt.radius}m
                                </span>
                            </div>
                        </div>
                    </div>
                    <div style={{ display: "flex", gap: 8, marginTop: 11 }}>
                        <button onClick={() => onOpenQR(site.id, pt.id)}
                            style={{
                                flex: 1, padding: "9px", borderRadius: 10, border: "none",
                                background: C.ok, color: "#fff", fontSize: 12.5, fontWeight: 700,
                                cursor: "pointer", fontFamily: "inherit",
                                display: "flex", alignItems: "center", justifyContent: "center", gap: 6
                            }}>
                            <QrCode size={14} /> Lihat QR
                        </button>
                        <button onClick={() => onEdit(site.id, pt.id)}
                            style={{
                                flex: 1, padding: "9px", borderRadius: 10,
                                border: `1.5px solid ${C.cardBd}`, background: "#fff",
                                color: C.mute, fontSize: 12.5, fontWeight: 700,
                                cursor: "pointer", fontFamily: "inherit",
                                display: "flex", alignItems: "center", justifyContent: "center", gap: 6
                            }}>
                            <Pencil size={13} /> Edit
            ...
            <button onClick={() => onAdd(site.id)}
                ...
                <Plus size={18} /> Tambah titik absensi
            </button>
        </div>
    );
}

function ScreenPointForm({ site, existing, onSave, onCancel }) {
    const [label, setLabel] = useState(existing?.label || "");
    const [address, setAddress] = useState(existing?.address || "");
    const [lat, setLat] = useState(existing?.lat ?? -6.2500);
    const [lng, setLng] = useState(existing?.lng ?? 106.6400);
    const [radius, setRadius] = useState(existing?.radius ?? RADIUS_DEFAULT);
    const [locating, setLocating] = useState(false);

    const useMyLocation = () => {
        setLocating(true);
        setTimeout(() => {
            setLat(-6.2705 + (Math.random() - 0.5) * 0.002);
            setLng(106.6290 + (Math.random() - 0.5) * 0.002);
            setLocating(false);
        }, 1200);
    };
    ...
    const applyAddress = () => {
        ...
        setLat(-6.2600 + (address.length % 10) * 0.001);
        setLng(106.6350 + (address.length % 8) * 0.001);
    };

    const canSave = label.trim() && !locating;

    return (
        <div>
            <div style={{ fontSize: 12.5, color: C.mute, marginBottom: 12, lineHeight: 1.5 }}>
                {site.name} · Tentukan lokasi titik. Pakai lokasi Anda sekarang, atau
                ketik alamat kalau setup dari jauh.
            </div>

            <MapRepresentation lat={lat} lng={lng} radius={radius} locating={locating} />

            <div style={{ display: "flex", gap: 8, marginTop: 12, marginBottom: 8 }}>
                <button onClick={useMyLocation} disabled={locating}
                    style={{
                        flex: 1, padding: "11px", borderRadius: 11, border: "none",
                        background: locating ? "#9DBBE0" : C.blue, color: "#fff",
                        fontSize: 13, fontWeight: 700, cursor: locating ? "default" : "pointer",
                        fontFamily: "inherit",
                        display: "flex", alignItems: "center", justifyContent: "center", gap: 7
                    }}>
                    <Crosshair size={16} /> {locating ? "Mendeteksi…" : "Pakai lokasi saya"}
                </button>
            </div>

            <label style={lblStyle2}>Atau ketik nama gedung / alamat</label>
            <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
                <input value={address} onChange={(e) => setAddress(e.target.value)}
                    placeholder="Mis. Gedung BP Legok, Jl. Raya Legok"
                    style={{ ...inpStyle2, flex: 1, marginBottom: 0 }} />
                <button onClick={applyAddress}
                    style={{
                        padding: "0 14px", borderRadius: 11, border: `1.5px solid ${C.blue}`,
                        background: C.blueBg, color: C.blue, fontSize: 12.5, fontWeight: 700,
            ...
            <label style={lblStyle2}>Nama titik <span style={{ color: C.danger }}>*</span></label>
            <input value={label} onChange={(e) => setLabel(e.target.value)}
                placeholder="Mis. Pos Utama Lobby"
                style={{ ...inpStyle2, marginBottom: 14 }} />

            <label style={lblStyle2}>
                Radius toleransi · <span style={{ color: C.blue, fontWeight: 800 }}>{radius} meter</span>
            </label>
            <div style={{ display: "flex", alignItems: "center", gap: 11, marginBottom: 6 }}>
                <button onClick={() => setRadius((r) => Math.max(RADIUS_MIN, r - RADIUS_STEP))}
                ...
                <button onClick={() => setRadius((r) => Math.min(RADIUS_MAX, r + RADIUS_STEP))}
                    style={stepBtn}><Plus size={16} /></button>
            </div>
            <div style={{ fontSize: 11, color: C.soft, marginBottom: 8, lineHeight: 1.5 }}>
                Absen sah kalau worker dalam radius ini saat scan QR. Radius = konteks
                tambahan; scan QR + foto tetap bukti utama.
            </div>

            <div style={{
                display: "flex", gap: 8, padding: "10px 12px", background: C.chip,
                borderRadius: 10, fontSize: 11, color: C.soft, lineHeight: 1.5, marginBottom: 16,
                border: `1px dashed ${C.cardBd}`
            }}>
                <Info size={14} style={{ flexShrink: 0, marginTop: 1 }} />
                <span>
                    Setelah disimpan, QR digenerate untuk titik ini. Cetak lalu tempel di
                    lokasi fisik — worker scan QR itu untuk absen.
                </span>
            </div>
            ...
            <div style={{ display: "flex", gap: 10 }}>
                <button onClick={onCancel}
                    style={{
                        ...
                        border: `1.5px solid ${C.cardBd}`, background: "#fff", color: C.mute,
                ...
                <button disabled={!canSave}
                    onClick={() => onSave(site.id, {
                        id: existing?.id, label: label.trim(), address: address.trim() || "—",
                        lat, lng, radius, qrReady: true
                    })}
                    style={actionBtn(canSave ? C.ok : "#A9DDCB")}>
                    <Check size={16} /> Simpan & buat QR
                </button>
            </div>
        </div>
    );
...
function ScreenPointQR({ site, point }) {
    if (!point) return <div style={{ padding: 20, color: C.mute }}>Titik tidak ditemukan.</div>;
    return (
        <div>
            <div style={{ textAlign: "center", marginBottom: 8 }}>
                <div style={{ fontSize: 15, fontWeight: 800, color: C.ink }}>{point.label}</div>
                <div style={{ fontSize: 12, color: C.mute, marginTop: 2 }}>{site.name}</div>
            </div>

            <div style={{
                background: "#fff", border: `1.5px solid ${C.cardBd}`,
                borderRadius: 16, padding: "22px", margin: "14px 0",
                ...
                <div style={{ textAlign: "center" }}>
                    ...
                    <div style={{
                        fontSize: 11, color: C.soft, marginTop: 3,
                        display: "flex", alignItems: "center", gap: 5, justifyContent: "center"
                    }}>
                        <Navigation size={10} /> {point.lat.toFixed(4)}, {point.lng.toFixed(4)} · radius {point.radius}m
                    </div>
                </div>
            </div>

            <div style={{
                display: "flex", gap: 8, padding: "11px 13px", background: C.blueBg,
                borderRadius: 11, fontSize: 11.5, color: C.blue, lineHeight: 1.5, marginBottom: 16
            }}>
                <QrCode size={15} style={{ flexShrink: 0, marginTop: 1 }} />
                <span>
                    QR ini membawa identitas titik sejak dicetak. Worker scan → sistem
                    langsung tahu "{point.label} · {site.name}". Cetak & tempel di lokasi.
                </span>
            </div>
            ...
            <div style={{ display: "flex", gap: 10 }}>
                <button
                    style={{
                        ...
                        background: C.ink, color: "#fff", fontWeight: 700, fontSize: 14,
                        cursor: "pointer", fontFamily: "inherit",
                        display: "flex", alignItems: "center", justifyContent: "center", gap: 7
                    }}>
...
function QRDummy() {
    ...
    return (
        <div style={{
            display: "grid",
            gridTemplateColumns: `repeat(${size}, 1fr)`, width: 180, height: 180,
            gap: 0, background: "#fff", padding: 6, borderRadius: 8,
            border: `1px solid ${C.cardBd}`
        }}>
            {cells.map((on, i) => (
                <div key={i} style={{ background: on ? C.ink : "#fff", aspectRatio: "1" }} />
            ))}
        </div>
    );
}
...
const stepBtn = {
    width: 40, height: 40, borderRadius: 10, border: `1.5px solid ${C.cardBd}`,
    background: "#fff", color: C.ink, cursor: "pointer", flexShrink: 0,
...
export default function App() {
    ...
    const [attendancePoints, setAttendancePoints] = useState(INITIAL_ATTENDANCE_POINTS);
    const top = stack[stack.length - 1];
    const push = (v) => setStack((s) => [...s, v]);
    const pop = () => setStack((s) => (s.length > 1 ? s.slice(0, -1) : s));

    // Simpan titik absensi: kalau ada id → edit, kalau tidak → tambah baru.
    // Setelah simpan, langsung buka QR titik itu (alur: simpan → QR).
    function saveAttendancePoint(siteId, point) {
        let savedId = point.id;
        setAttendancePoints((all) => {
            const list = all[siteId] || [];
            if (point.id) {
                return { ...all, [siteId]: list.map((p) => p.id === point.id ? { ...p, ...point } : p) };
            }
            savedId = `ap-${Date.now()}`;
            return { ...all, [siteId]: [...list, { ...point, id: savedId }] };
        });
        // Ganti layar form dgn QR titik yg baru disimpan
        setStack((s) => [...s.slice(0, -1), { view: "point_qr", siteId, pointId: savedId }]);
    }
    ...
    const title = (() => {
        ...
        if (top.view === "attendance_points") return "Titik Absensi";
        if (top.view === "site_points") return SITES.find((s) => s.id === top.siteId)?.name || "Titik Absensi";
        if (top.view === "point_form") return top.pointId ? "Edit Titik" : "Tambah Titik";
        if (top.view === "point_qr") return "QR Titik Absensi";
        return "";
    })();
        ...
        *::-webkit-scrollbar{width:0}`}</style>
            ...
            <div style={{
                ...
                <div style={{ flex: 1, overflowY: "auto", padding: 18, position: "relative" }}>
                    ...
                    {top.view === "attendance_points" && (
                        <ScreenAttendancePoints points={attendancePoints}
                            onOpenSite={(siteId) => push({ view: "site_points", siteId })} />
                    )}
                    {top.view === "site_points" && (
                        <ScreenSitePoints site={SITES.find((s) => s.id === top.siteId)}
                            points={attendancePoints}
                            onAdd={(siteId) => push({ view: "point_form", siteId })}
                            onEdit={(siteId, pointId) => push({ view: "point_form", siteId, pointId })}
                            onOpenQR={(siteId, pointId) => push({ view: "point_qr", siteId, pointId })} />
                    )}
                    {top.view === "point_form" && (
                        <ScreenPointForm site={SITES.find((s) => s.id === top.siteId)}
                            existing={top.pointId
                                ? (attendancePoints[top.siteId] || []).find((p) => p.id === top.pointId)
                                : null}
                            onSave={saveAttendancePoint} onCancel={pop} />
                    )}
                    {top.view === "point_qr" && (
                        <ScreenPointQR site={SITES.find((s) => s.id === top.siteId)}
                            point={(attendancePoints[top.siteId] || []).find((p) => p.id === top.pointId)} />
                    )}
                </div>
            </div>
        </div>
