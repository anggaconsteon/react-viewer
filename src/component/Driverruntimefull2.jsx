...
const C = {
    ...
    // SALE (ownership transfer) — teal, distinct from drop(indigo)/pickup(violet)
    sale50: "#f0fdfa",
    sale100: "#ccfbf1",
    sale400: "#2dd4bf",
    sale600: "#0d9488",
...
const INITIAL_TASKS = [
    ...
    },
    {
        ...
        state: "assigned",
        items: [
            { id: "gas_12", name: "Gas 12kg", type: "returnable", transactionType: "SALE", condition: "full", planDrop: 3, actualDrop: 0, planPickup: 0, actualPickup: 0 },
            { id: "aqua_galon", name: "Aqua Galon", type: "returnable", planDrop: 6, actualDrop: 0, planPickup: 0, actualPickup: 0 },
            { id: "pristine_galon", name: "Pristine Galon", type: "returnable", transactionType: "REFILL", planRefill: 2, waterType: "ro", planDrop: 0, actualDrop: 0, planPickup: 0, actualPickup: 0 },
        ],
    },
    {
        id: "T-053",
        ...
        items: [
            ...
            { id: "aqua_galon", name: "Aqua Galon", type: "returnable", transactionType: "SALE", condition: "full", planDrop: 4, actualDrop: 0, planPickup: 0, actualPickup: 0 },
            { id: "amidis_galon", name: "Amidis Galon", type: "returnable", transactionType: "PURCHASE", planDrop: 0, actualDrop: 0, planPickup: 0, actualPickup: 0, planBuy: 4, condition: "empty" },
        ],
    },
];

const DRIVER = { name: "Budi Santoso", id: "DRV-001", role: "Driver" };  // executor (resolved dari scan kartu)
const VEHICLE = { id: "V-007", plate: "B 1234 XY" };
const DEMO_PIN = "2468";
// Pemilik HP di skenario demo — HP ini punya Andi (lagi login). Budi mau minjam.
const DEVICE_OWNER = { name: "Andi Wijaya", id: "DRV-002", role: "Driver" };
...
const Chip = ({ children, variant = "neutral" }) => {
    const variants = {
        ...
        sale: { bg: C.sale100, fg: C.sale700 },
...
const ItemExecutionRow = ({ item, dropValue, pickupValue, onChangeDrop, onChangePickup }) => {
    ...
    const isRefill = item.transactionType === "REFILL";
    const hasDrop = item.planDrop > 0;
    const showPickupStepper = isReturnable && !isSale && !isPurchase && !isRefill;

    // SALE — ownership transfer. Read-only "Jual {qty}", no steppers, no pickup.
    // Driver tidak memutuskan/mengubah ini (ditetapkan Admin). Tinggal eksekusi.
    if (isSale) {
        return (
            <div style={{
                background: C.surface,
                ...
                <div style={{
                    ...
                    background: C.sale50, border: `1.5px solid ${C.sale100}`,
                    borderRadius: 10, padding: "12px 14px",
                }}>
                    <span style={{ color: C.sale600, fontSize: 18, fontWeight: 700 }}>→</span>
                    <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 10, color: C.textMid, textTransform: "uppercase", letterSpacing: "0.04em", fontWeight: 700 }}>
                            Jual ke customer
                        </div>
                        <div style={{ fontSize: 11, color: C.sale700, marginTop: 1 }}>Kepemilikan pindah · tanpa pickup</div>
                    </div>
                    <span className="mono" style={{ fontSize: 28, fontWeight: 700, color: C.sale700, lineHeight: 1 }}>
                        {item.planDrop}
                    </span>
                </div>
            </div>
    ...
    // PURCHASE — inbound mirror of SALE. Read-only "Beli {qty}", no steppers.
    // Unit masuk dari customer; ownership pindah ke operator. Driver tinggal eksekusi.
    if (isPurchase) {
        return (
            <div style={{
                background: C.surface,
                ...
                <div style={{
                    ...
                    <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 11, color: C.sale700, marginTop: 1 }}>Kepemilikan ke operator · naik ke kendaraan</div>
                    </div>
                    <span className="mono" style={{ fontSize: 28, fontWeight: 700, color: C.sale700, lineHeight: 1 }}>
                        {item.planBuy}
                    </span>
    ...
    // REFILL — content/service. Galon milik customer ditukar 1-lawan-1 (kosong in /
    // isi out). Read-only; driver tinggal eksekusi tukar. Bukan pickup, bukan jual.
    if (isRefill) {
        return (
            <div style={{
                background: C.surface,
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10, paddingLeft: 2 }}>
                    <span style={{ fontSize: 14, fontWeight: 700, color: C.text }}>{item.name}</span>
                    <Chip variant="emerald">Refill</Chip>
                    {item.waterType && (
                        <span style={{
                            padding: "1px 5px", borderRadius: 3,
                            background: C.slate100, color: C.slate600,
                ...
                <div style={{
                    display: "flex", alignItems: "center", gap: 10,
                    background: C.emerald50, border: `1.5px solid ${C.emerald100}`,
                    borderRadius: 10, padding: "12px 14px",
                }}>
                    <span style={{ color: C.emerald700, fontSize: 18, fontWeight: 700 }}>⇄</span>
                    <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 10, color: C.textMid, textTransform: "uppercase", letterSpacing: "0.04em", fontWeight: 700 }}>
                            Tukar galon customer
                        </div>
                        <div style={{ fontSize: 11, color: C.emerald700, marginTop: 1 }}>Kosong masuk · isi keluar · galon milik customer</div>
                    </div>
                    <span className="mono" style={{ fontSize: 28, fontWeight: 700, color: C.emerald700, lineHeight: 1 }}>
                        {item.planRefill}
                    </span>
                </div>
            </div>
...
// SCREEN: TASK FEED
// ═══════════════════════════════════════════════════════════════════════════
const TaskFeedScreen = ({ tasks, onSelectTask, recentlyCompletedId, onBack }) => {
...
const DeliveryExecutionWorkspace = ({ task, onBack, onSubmit }) => {
    ...
    const canConfirm = totalDrop > 0 || totalPickup > 0 ||
        task.items.some(i => i.transactionType === "SALE" || i.transactionType === "PURCHASE" || i.transactionType === "REFILL");
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "8px 18px 16px" }} className="scroll-thin">
                <div style={{
                    ...
                    {task.items.map((item, idx) => {
                        ...
                        const isPurchase = item.transactionType === "PURCHASE";
                        const isRefill = item.transactionType === "REFILL";
                        const showPickupRow = isReturnable && !isSale && !isPurchase && !isRefill && (item.planPickup > 0 || actPickup > 0);
                                ...
                                <div style={{ display: "flex", gap: 12, fontSize: 12, flexWrap: "wrap" }}>
                                    {isSale && (
                                        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                                            ...
                                            <span className="mono" style={{ color: C.sale700, fontWeight: 700 }}>{item.planDrop}</span>
                                            <span style={{ color: C.textDim, fontSize: 11 }}>jual · tanpa pickup</span>
                                        </div>
                                    )}
                                    {isPurchase && (
                                        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                                            <span style={{ color: C.sale600, fontWeight: 700 }}>←</span>
                                            <span className="mono" style={{ color: C.sale700, fontWeight: 700 }}>{item.planBuy}</span>
                                            <span style={{ color: C.textDim, fontSize: 11 }}>beli · masuk kendaraan</span>
                                        </div>
                                    )}
                                    {isRefill && (
                                        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                                            <span style={{ color: C.emerald700, fontWeight: 700 }}>⇄</span>
                                            <span className="mono" style={{ color: C.emerald700, fontWeight: 700 }}>{item.planRefill}</span>
                                            <span style={{ color: C.textDim, fontSize: 11 }}>tukar · kosong in / isi out</span>
                                        </div>
                                    )}
                                    {!isSale && item.planDrop > 0 && (
                                        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
...
// ═══════════════════════════════════════════════════════════════════════════
const DeviceOwnerScreen = ({ owner, onHandover }) => {
...
const ScanScreen = ({ onScanned, resuming = false, executorName = "", remaining = 0 }) => (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", background: C.bg }}>
        <div style={{ background: C.surface, borderBottom: `1px solid ${C.border}`, padding: "14px 16px", flexShrink: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{
...
// ─── AUTH GATE — PIN ────────────────────────────────────────────────────────
const PinScreen = ({ executor, onBack, onSuccess, resuming = false }) => {
...
const PauseConfirmSheet = ({ remaining, onConfirm, onCancel }) => (
    <>
        <div onClick={onCancel} style={{ position: "absolute", inset: 0, background: "rgba(15, 23, 42, 0.5)", zIndex: 10 }} />
        <div className="sheet-up" style={{
...
const CustodyNotificationScreen = ({ custody, onStartConfirmation, onBack }) => {
    return (
        <div style={{ height: "100%", display: "flex", flexDirection: "column", background: C.bg }}>
...
const IndependentCountWorkspace = ({ custody, onBack, onContinue }) => {
...
// ═══════════════════════════════════════════════════════════════════════════
const ConfirmationSuccessScreen = ({ custody, counts, onProceed }) => {
    return (
        <div style={{ height: "100%", display: "flex", flexDirection: "column", background: C.bg }}>
            <div style={{
                background: C.surface,
...
// ═══════════════════════════════════════════════════════════════════════════
const MismatchReportScreen = ({ custody, counts, onBack, onSubmit }) => {
    const [note, setNote] = useState("");
    const [hasPhoto, setHasPhoto] = useState(false);
...
// ═══════════════════════════════════════════════════════════════════════════
const MismatchSubmittedScreen = ({ custody, onDone }) => {
    return (
        <div style={{ height: "100%", display: "flex", flexDirection: "column", background: C.bg }}>
            <div style={{
                background: C.surface,
...
const HomeRouteCard = ({ tasks, locked, onOpenFeed, onRejectTask }) => {
    const total = tasks.length;
    const completed = tasks.filter(t => t.state === "completed").length;
    const failed = tasks.filter(t => t.state === "failed").length;
        ...
        return (
            <div style={{ background: C.surface, border: `1px solid ${C.borderStrong}`, borderRadius: 14, marginBottom: 14, overflow: "hidden" }}>
                ...
                <div style={{ padding: "6px 8px 8px" }}>
                    {tasks.map((t) => (
                        <div key={t.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 8px" }}>
                            <div style={{
                            ...
                            {t.state === "completed"
                                ? <Chip variant="emerald">Selesai</Chip>
                                : onRejectTask && (
                                    <button onClick={() => onRejectTask(t)} className="tap-feedback" style={{
                                        flexShrink: 0, background: "transparent", border: `1px solid ${C.amber400}`,
                                        color: C.amber700, fontSize: 10.5, fontWeight: 700, padding: "5px 9px",
                ...
                <div style={{ padding: "0 14px 10px", fontSize: 10.5, color: C.textDim, lineHeight: 1.4 }}>
                    Ada stop yang nggak searah? <strong style={{ color: C.amber700 }}>Tolak</strong> sebelum berangkat — dikembalikan ke Admin buat mobil lain. Setelah berangkat, nggak bisa.
                </div>
            </div>
        );
    }
...
const HomeView = ({ tasks, custodyStatus, custodyCounts, onConfirmCustody, onOpenFeed, onLogout, onReturn, onRejectTask }) => {
    const total = tasks.length;
    ...
    return (
        <div style={{ height: "100%", display: "flex", flexDirection: "column", background: C.bg }}>
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "14px 16px 20px" }} className="scroll-thin">
                ...
                <HomeRouteCard tasks={tasks} locked={!confirmed} onOpenFeed={onOpenFeed} onRejectTask={!confirmed ? onRejectTask : undefined} />
...
const ReturnScreen = ({ tasks, onBack, onConfirm }) => {
...
// ─── REJECT TASK SHEET (tolak — tidak searah, opening-only) ────────────────
const RejectTaskSheet = ({ task, onConfirm, onCancel }) => {
    ...
    return (
        <>
            ...
            <div className="sheet-up" style={{
                ...
                borderTopLeftRadius: 18, borderTopRightRadius: 18, zIndex: 21, padding: "10px 16px 18px",
            }}>
                <div style={{ display: "flex", justifyContent: "center", paddingBottom: 10 }}>
                    <div style={{ width: 40, height: 4, background: C.slate200, borderRadius: 2 }} />
                </div>
                <div style={{ fontSize: 16, fontWeight: 700, color: C.text, marginBottom: 2 }}>Tolak Task — Tidak Searah</div>
                <div style={{ fontSize: 12, color: C.textMid, marginBottom: 12 }}>{task.customer} · Stop {task.stopNumber}</div>

                <div style={{
                    background: C.amber50, border: `1px solid ${C.amber400}33`, borderLeft: `3px solid ${C.amber400}`,
                    borderRadius: 10, padding: "10px 12px", fontSize: 11.5, color: C.amber700, lineHeight: 1.45, marginBottom: 14,
                }}>
                    Task ini dikembalikan ke <strong>Admin</strong> buat di-assign ke mobil lain. Barang tetap di gudang — nggak naik ke kendaraan lo. Cuma bisa <strong>sebelum berangkat</strong>.
                </div>

                <div style={{ fontSize: 11, color: C.textDim, textTransform: "uppercase", letterSpacing: "0.05em", fontWeight: 700, marginBottom: 6 }}>Alasan (wajib)</div>
                <textarea
                    value={reason}
                    onChange={(e) => setReason(e.target.value)}
                    placeholder="Kenapa nggak searah? mis. arah berlawanan dari rute, kejauhan dari titik lain…"
                    rows={3}
                    style={{
                        width: "100%", boxSizing: "border-box", padding: "11px 12px", border: `1px solid ${C.border}`,
                        borderRadius: 10, fontSize: 13, color: C.text, outline: "none", resize: "none", fontFamily: "inherit", lineHeight: 1.4,
                    }}
                />
                <div style={{ fontSize: 10.5, color: ok ? C.emerald700 : C.textDim, marginTop: 4, marginBottom: 14 }}>
                    {ok ? "✓ alasan cukup" : `min 10 karakter · ${reason.trim().length}/10`}
                </div>

                <div style={{ display: "flex", gap: 10 }}>
                    <button onClick={onCancel} className="tap-feedback" style={{
                        flex: 1, background: "transparent", border: `1px solid ${C.border}`, color: C.textMid,
                        padding: "13px", borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: "pointer",
                    }}>Batal</button>
                    <button onClick={() => ok && onConfirm(reason.trim())} disabled={!ok} className="tap-feedback" style={{
                        flex: 2, background: ok ? C.amber700 : C.slate200, color: ok ? "#fff" : C.textDim, border: "none",
                        padding: "13px", borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: ok ? "pointer" : "not-allowed",
...
export default function DriverRuntimeFull() {
    ...
    const [rejectingTask, setRejectingTask] = useState(null);     // task yang lagi mau ditolak (opening-only)
    const [rejectToast, setRejectToast] = useState(null);         // konfirmasi bounce

    const pendingCount = tasks.filter(t => t.state === "assigned").length;
    const activeTask = tasks.find(t => t.id === activeTaskId);

    // Load rejection (opening-only): task di-bounce dari muatan → balik ke Admin
    // buat di-assign ke mobil lain. Bukan movement, bukan discrepancy. Barang tetap di gudang.
    const handleRejectTask = (reason) => {
        const t = rejectingTask;
        if (!t) return;
        // emit task_load_rejected (lifecycle) → task lepas dari muatan driver
        setTasks(prev => prev.filter(x => x.id !== t.id));
        setRejectingTask(null);
        setRejectToast(`${t.customer} dikembalikan ke Admin · ga searah`);
        setTimeout(() => setRejectToast(null), 2800);
    };
                    ...
                    <div style={{ height: "calc(100% - 36px)", position: "relative" }}>
                        ...
                        {session && screen === "home" && (
                            <HomeView
                                ...
                                onRejectTask={(t) => setRejectingTask(t)}
                            />
                        )}
                        ...
                        {/* LOAD REJECTION — tolak task tidak searah (opening-only) */}
                        {rejectingTask && (
                            <RejectTaskSheet
                                task={rejectingTask}
                        ...
                        {rejectToast && (
                            <div style={{
                                ...
                                <span style={{ flex: 1 }}>{rejectToast}</span>
                            </div>
                        )}
                    </div>
                </div>
...
// ─── Named exports (for DriverRuntimeGallery2) ──────────────────────────────
export {
    FontLoader,
    INITIAL_TASKS,
    DRIVER,
